...
namespace ScriptStack
{
    public class YAML : Model
    {
        ...
        public YAML()
        {
            ...
            routines.Add(new Routine((Type)null, "yaml.parse", (Type)null, "Erstelle ein YAML Objekt (YamlNode) aus einem String."));
            routines.Add(new Routine((Type)null, "yaml.get", (Type)null, (Type)null, "Lese aus YAML per Path (a.b[0].c)."));
            routines.Add(new Routine((Type)null, "yaml.set", (Type)null, (Type)null, (Type)null, "Setze in YAML per Path (a.b[0].c)."));

            routines.Add(new Routine(typeof(int), "yaml.count", (Type)null, "Anzahl Elemente (Sequence) / Properties (Mapping)."));
            routines.Add(new Routine(typeof(ScriptStack.Runtime.ArrayList), "yaml.keys", (Type)null, "Gibt alle Keys eines YAML-Mappings als array zurück."));
            routines.Add(new Routine(typeof(bool), "yaml.has", (Type)null, (Type)null, "Prüft ob ein YAML-Path existiert (und nicht null ist)."));

            routines.Add(new Routine((Type)null, "yaml.iter", (Type)null, "Iterator für YamlMapping/YamlSequence (oder raw yaml string)."));
            routines.Add(new Routine(typeof(bool), "yaml.next", (Type)null, "Iterator: nächstes Element. true wenn vorhanden."));
            routines.Add(new Routine(typeof(string), "yaml.key", (Type)null, "Iterator: aktueller Key (Mapping) oder null (Sequence)."));
            routines.Add(new Routine(typeof(int), "yaml.index", (Type)null, "Iterator: aktueller Index (Sequence) oder -1 (Mapping)."));
            routines.Add(new Routine((Type)null, "yaml.value", (Type)null, "Iterator: aktueller Wert (primitive oder YamlNode)."));

            routines.Add(new Routine((Type)null, "yaml.node", (Type)null, "Konvertiert Wert/ArrayList zu YamlNode (Mapping/Sequence/Scalar)."));
            routines.Add(new Routine(typeof(string), "yaml.string", (Type)null, "Konvertiert Wert/ArrayList/YamlNode zu YAML-String."));

            routines.Add(new Routine(typeof(bool), "yaml.is_map", (Type)null, "True wenn Node ein Mapping/Object ist."));
            routines.Add(new Routine(typeof(bool), "yaml.is_seq", (Type)null, "True wenn Node eine Sequence/Array ist."));
        ...
        private static YamlNode ParseYaml(string yaml)
        {
            ...
            if (stream.Documents.Count == 0)
                throw new InvalidOperationException("yaml.parse: Keine YAML Dokumente gefunden.");
        ...
        private static YamlNode CoerceToNode(object? p0)
        {
            ...
            throw new InvalidOperationException("yaml.iter erwartet YamlNode oder YAML-String.");
        }
        ...
        private sealed class YamlIterator
        {
            ...
            public void Reset()
            {
                ...
                if (_root is YamlMappingNode map)
                    ...
                    throw new InvalidOperationException("yaml.iter erwartet YamlMappingNode oder YamlSequenceNode.");
            }
